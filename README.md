# ObsidianGuard

ObsidianGuard is a quantum-resistant, post-quantum cryptography library for secure digital interactions.

# Building

- Make sure to use `ninja` generator to generate compile_commands.json in build folder
- `cmake -S . -B build -G Ninja` to generate build files
- `cmake --build build` to build the project

## Naming Conventions

To keep the C++ codebase clean, readable, and consistent, all developers should follow a simple naming style throughout the project.

1. **Class names** must use `PascalCase`, meaning each word starts with a capital letter, such as `MyClass` or `UserManager`.
2. **Methods** must use `camelCase`, starting with a lowercase letter, for example `loadData()` or `calculateScore()`.
3. **Variables** must also use `camelCase`, starting with a lowercase letter, such as `userCount`, `filePath`, or `totalScore`.
4. This applies to both local variables and member variables. Consistency in these rules is required across the entire codebase to ensure clarity and maintainability.

# Authors

- [Mostafa Maher](https://github.com/devostafa)
- [Vuk Amizic](https://github.com/Amizic)
